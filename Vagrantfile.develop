...
ENV = YAML.load_file('vars/env.yml')
SHELL_PATH = 'vagrant_shells/'

ssh_pub_key_string = File.readlines(ENV["id_rsa"]).first.strip
...
Vagrant.configure(2) do |config|
  ...
  NODES.each do |node|
    config.vm.define ENV["name"] + "-" + node["name"] do |subconfig|
      ...
      subconfig.vm.hostname = ENV["name"] + "-" + node["name"]  
      subconfig.vm.box = node["box"]
      # subconfig.vm.provision :shell, path: "bootstrap_ansible.sh"

      shells = node["shells"]
      shells.each do |shell|
        subconfig.vm.provision "shell", path: SHELL_PATH + shell["path"], args: [ssh_pub_key_string]
      end
      ...
      subconfig.vm.provider "virtualbox" do |vb|
        vb.name = ENV["name"] + "-" + node["name"] # virtual box 的顯示名稱
        vb.gui = false
        vb.cpus = node["cpus"]
